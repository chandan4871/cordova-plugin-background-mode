...
and caches the results with chart data using SDE connection.
...
class InfraTaggingProcessor:
    ...
    def __init__(self, sde_path: str, app_schema: str, log_folder: str = None, email_config: Dict = None):
        """
        Initialize the processor
        
        Args:
            sde_path: Path to SDE connection file
            app_schema: Database schema name (e.g., 'ONETOOLAPP.')
            log_folder: Path to log folder
            email_config: Email configuration dictionary
        """
        self.sde_path = sde_path
        self.app_schema = app_schema.rstrip('.') + '.'  # Ensure trailing dot
    ...
    def execute_sql(self, sql: str, return_result: bool = True):
        """
        Execute SQL against ArcGIS SDE via ArcPy
        
        Args:
            sql: SQL query to execute
            return_result: Whether to return results
        ...
        try:
            ...
            if result == True:
                ...
                # For SELECT operations - returns list of tuples
                return result
            else:
            ...
            self.log(f"SQL: {sql[:200]}...")  # Log first 200 chars of SQL
            raise
    ...
    def send_status_email(self, status: str):
        ...
        try:
            if not self.email_config or not self.email_config.get('enabled', False):
                self.log("Email configuration not enabled, skipping email notification")
                return
...
def main():
    ...
    try:
        ...
        sde_path = arcpy.GetParameterAsText(0)  # SDE connection file path
        app_schema = arcpy.GetParameterAsText(1)  # Database schema name (e.g., 'ONETOOLAPP.')
        log_folder = arcpy.GetParameterAsText(2)  # Log folder path (optional)
        
        # Optional email parameters
        smtp_server = arcpy.GetParameterAsText(3)  # SMTP server
        smtp_port = arcpy.GetParameter(4) if arcpy.GetParameterAsText(4) else 587  # SMTP port
        from_email = arcpy.GetParameterAsText(5)  # From email address
        to_emails = arcpy.GetParameterAsText(6)  # To email addresses (comma-separated)
        email_username = arcpy.GetParameterAsText(7)  # Email username (optional)
        email_password = arcpy.GetParameterAsText(8)  # Email password (optional)
